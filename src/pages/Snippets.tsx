         ...
         <div className="space-y-4">
            {filteredSnippets.length === 0 ? (
               ...
               filteredSnippets.map((snippet) => (
                  <div
                     key={snippet.id}
                     className={`bg-slate-900 rounded-xl border transition-all overflow-hidden min-w-0 ${expandedId === snippet.id ? 'border-blue-500/50 shadow-lg' : 'border-slate-800 hover:border-slate-700'}`}
                  >
                     <div
                     ...
                     {expandedId === snippet.id && (
                        <div className="border-t border-slate-800 bg-slate-950 p-4 animate-fade-in overflow-hidden">
                           <div className="relative group min-w-0">
                              <pre className="bg-[#0B1021] p-4 rounded-lg overflow-x-auto text-sm font-mono text-slate-300 border border-slate-800/50 max-h-96 custom-scrollbar dir-ltr text-left w-full max-w-full">
